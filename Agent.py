...
class Agent:
    ...
    def get_state(self, vision):
        """Generate the current state based on vision input."""
    ...
    def choose_action(self, state, valid_actions):
        ...
        if not q_values:
            q_values = {action: -100.0 for action in DIRECTIONS.keys()}
        ...
        best_actions = [
            a for a in valid_actions if noisy_q_values[a] == max_q
        ]
    ...
    def save_model(self, filename):
        """Save the Q-table to a file with proper encoding."""
        try:
    ...
    def load_model(self, filename):
        """Load the Q-table from a file and validate the data."""
        try:
            ...
            for state_str, actions in self.q_table.items():
                ...
                try:
                    ...
                    if (
                        ...
                        and len(state) == 12
                        and all(isinstance(x, bool) for x in state)
                    ):
                        # Verify actions are valid
                        valid_actions = {}
                        for action, value in actions.items():
                            if action in DIRECTIONS and \
